...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_POST['id'])) {
        $stmt = $conn->prepare("UPDATE customers SET name=?, phone=?, email=?, address=?, type=? WHERE id=?");
        $stmt->bind_param("sssssi", $name, $phone, $email, $address, $type, $_POST['id']);
    } else {
        $stmt = $conn->prepare("INSERT INTO customers (name, phone, email, address, type) VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("sssss", $name, $phone, $email, $address, $type);
    }
